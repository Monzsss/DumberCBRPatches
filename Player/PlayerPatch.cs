...
{
    public static class ModSettingsDataRegisterx
    {
        ...
        private static void BuildCharacterSettings()
        {
            ...
            foreach (var name in TargetCharacters)
            {
                ...
                // Generates UI component under explicit clean format configuration titles
                var tits = Add(new ModSettingsDataDropdown(
                    $"{displayName} Breast Size",
                    CustomTitsSizes,
                    defaultIndex,
                    $"Adjust {displayName}'s volumetric base size configuration"
                ));

                var traits = Add(new ModSettingsDataString(
                    $"{displayName} Essence",
                    defaultTraitsString,
                    $"Adjust {displayName}'s internal attribute parameters"
                ));
